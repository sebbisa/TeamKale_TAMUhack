...
color = (255, 255, 255)
smallfont = pygame.font.SysFont('monospace', 35, bold=False, italic=False)
...
def subract_time(subtract_secs = 1):
    global seconds, minutes
    if seconds - subtract_secs <= 0:
        minutes -= 1
    seconds = (seconds - subtract_secs) % 60
    print(f'{minutes}:{seconds}')

def question_asked_gets_false():
...
def game():
    global newThread, prev_time
    global runGame, question_asked
    current_time = time.time()
    if current_time - prev_time >= 1:
        subract_time()
        prev_time = current_time
        print(f"{current_time}, {prev_time}")
    ...
    if not question_asked: 
        if newThread is not None:
            newThread.join()
        question_asked = True
        newThread = threading.Thread(target=ask_question, args = (2,), daemon=True)
        newThread.start()
